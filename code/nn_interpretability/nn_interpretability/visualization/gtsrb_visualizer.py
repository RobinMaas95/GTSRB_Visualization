...
class GTSRBVisualizer(LRPHeatmap):
    @staticmethod
    def show(image):
    ...
    def show_dataset_examples(trainloader, n: int = 1):
        ...
        images = images[0:n]
        GTSRBVisualizer.show_examples(images)
    ...
    def display_images_for_each_class(images):
      ...
      for i in range(rows):
          ...
          for j in range(2):
              plt.subplot(9, colums, i * colums + j + 1)
              plt.imshow(images[i * 5 + j].permute(1, 2, 0))
              plt.title('Class {}'.format(i * colums + j))
              plt.xticks([])
              plt.yticks([])
          plt.tight_layout()
    ...
    def display_images_with_probabilities_for_each_class(images, probabilities):
      ...
      for i in range(rows):
          ...
          for j in range(colums):
              ...
              plt.subplot(9, colums, i * colums + j + 1)
              plt.imshow(images[i * 5 + j].permute(1, 2, 0))
              plt.title('Class {}, Probability {:.2f}'.format(i * colums + j, probabilities[i * colums + j]))
              plt.xticks([])
              plt.yticks([])
    ...
    def cat_images(images, sx, sy):
        ...
        imgs_stack = np.hstack(image.squeeze(0).cpu().numpy().reshape(48, 48) for image in images)
    ...
    def result_for_each_rotation(images, cls, score, top_k, sx, sy):
        ...
        for i in range(len(images)):
            ...
            plt.imshow(images[i].reshape(48, 48), cmap='gray', vmin=0, vmax=1)
            plt.axis('off')
            plt.tight_layout()
